...
#define cellDistance 933 //in counts of the encoder
...
void moveForward() {
  ...
  while (1) {
    readDistance();
    in = cellDistance - countRight;
    pidEncoder->Compute();
    //    if (distance[FRONT_L] < 80 || distance[FRONT_L] < 80 || count > 10 ) break;
    if (distance[FRONT_L] < 80 || distance[FRONT_R] < 80 || count > 10 )
    {
...
void saveToMemory() {
  for (int i = 0; i < 256; i++) {
    eeprom.writeByte(shortPath[i]);
    if (//initial value maa aayo bhane) {
      shortPath[i] = 'Z';
      break;
  }
}
